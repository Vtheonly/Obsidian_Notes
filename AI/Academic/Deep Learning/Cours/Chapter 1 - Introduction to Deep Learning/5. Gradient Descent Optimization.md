...
**Why subtract?** If $f'(x_i) > 0$, the function is increasing at $x_i$. To find the minimum, we must move left (decrease $x$). Subtracting a positive number decreases $x$. If $f'(x_i) < 0$, the function is decreasing. We must move right (increase $x$). Subtracting a negative number increases $x$. This elegant mathematical rule ensures we always move downhill regardless of which side of the minimum we are on.
