...
```mermaid
graph TD
    Req["Incoming API Request"] --> IsAuthenticatedCheck{Global Authentication Checks}
    IsAuthenticatedCheck -->|Pass| CustomClass["Custom BasePermission"]
    
    subgraph "Custom Permission Class Evaluation"
        CustomClass --> GlobalCheck{Evaluate has_permission}
        GlobalCheck -->|Returns True| DetailCheck{Is single record targeted?}
        GlobalCheck -->|Returns False| Fail["HTTP 403 Forbidden Response"]
        
        DetailCheck -->|Yes| EvaluateObj{Evaluate has_object_permission}
        DetailCheck -->|No| Success["Execute API View Handler"]
        
        EvaluateObj -->|Returns True| Success
        EvaluateObj -->|Returns False| Fail
    end
```
