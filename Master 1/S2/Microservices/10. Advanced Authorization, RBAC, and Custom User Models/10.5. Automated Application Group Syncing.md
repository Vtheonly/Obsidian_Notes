...
```mermaid
graph TD
    Boot["Django Server Startup Process"] --> ReadyCall["Trigger AppConfig.ready"]
    ReadyCall --> QueryDB{Validate Role Groups DB Table}
    
    subgraph "Group Sync Pipeline"
        QueryDB -->|Check roles defined in choices| CreateGroup["Group.objects.get_or_create"]
        CreateGroup --> BindPerms["Assign default permissions to Group"]
    end
    
    BindPerms --> Active["Server is ready for incoming requests"]
```
