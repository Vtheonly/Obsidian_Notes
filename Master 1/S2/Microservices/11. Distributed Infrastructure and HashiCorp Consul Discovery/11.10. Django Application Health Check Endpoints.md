...
```mermaid
graph TD
    Probe["Consul Health Check Probe"] --> View["health_check View"]
    
    subgraph "Internal Checks"
        View --> DBCheck{Query Database}
        DBCheck -->|Success| CacheCheck{Verify Redis Cache}
        DBCheck -->|Failure: Database Offline| Fail["Return HTTP 503 Service Unavailable"]
        CacheCheck -->|Success| Pass["Return HTTP 200 OK"]
        CacheCheck -->|Failure: Cache Offline| Fail
    end
```
