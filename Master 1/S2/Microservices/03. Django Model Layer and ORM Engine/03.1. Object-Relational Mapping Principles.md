...
```mermaid
graph TD
    subgraph "Python Application (Object Space)"
        Class["Python Model Class"]
        Instance["Object Instance"]
        Attribute["Object Attributes"]
    end
    
    subgraph "Django ORM Engine (Translation Layer)"
        ORM["Query Compiler & SQL Generator"]
    end
    
    subgraph "Relational Database (SQL Space)"
    ...
    Class -->|Maps to| Table
    Instance -->|Maps to| Row
    Attribute -->|Maps to| Column
    
    Instance -->|Saves/Queries via| ORM
    ORM -->|Executes SQL| Table
```
