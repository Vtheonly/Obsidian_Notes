...
while IFS= read -r folder; do
  # Define the 'Lessons' and 'Active Recall Test' subfolder paths
  ...
  mkdir -p "$lesson_folder" "$active_recall_folder"
  ...
  if [ -z "$files" ]; then
    ...
    while IFS= read -r file; do
      ...
      if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
        ...
        mv "$file" "$lesson_folder/"
        echo "Moved $file to '$lesson_folder/'"
      else
        echo "Skipped $file"
      fi
    done <<< "$files"
