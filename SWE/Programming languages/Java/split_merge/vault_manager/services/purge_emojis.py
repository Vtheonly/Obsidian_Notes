...
Removes all emoji characters from text content. The emoji regex is
comprehensive — it covers every Unicode emoji range including
pictographs, dingbats, flags (regional indicators), variation
selectors, zero-width joiners, and tag sequences.

The service is integrated into the Vault Manager pipeline via the
:class:`EmojiPurgeService` class, which implements
:class:`IContentTransformer`. The static helpers :meth:`count_emojis`
and :meth:`is_text_file` are used by the CLI's purge-emojis command to
decide which files to scan and to report statistics.

Binary-file safety
-------------------
``is_text_file`` uses two strategies to avoid corrupting binaries:

1. Extension whitelist — known-text extensions (.md, .py, .json, etc.)
   are accepted immediately.
2. Filename whitelist — known-text filenames without extensions
   (Makefile, Dockerfile, etc.) are accepted.
3. For files with no extension, the first 1024 bytes are inspected for
   null bytes; presence of a null byte indicates a binary file.
"""
...
from vault_manager.logging import logger  # noqa: F401  (preserved for parity)


# ---------------------------------------------------------------------------
# Comprehensive emoji regex — preserved EXACTLY from the original.
# Do NOT modify. The order of ranges matters for some edge cases.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Text-file detection tables — preserved EXACTLY from the original.
# ---------------------------------------------------------------------------
...
class EmojiPurgeService(IContentTransformer):
    ...
    def transform(self, content: str) -> str:
        """Strip every emoji character from ``content``."""
        return EMOJI_PATTERN.sub("", content)

    @staticmethod
    def count_emojis(content: str) -> int:
        """Return the total number of emoji characters in ``content``.

        Each regex match is a run of one or more emoji characters; the
        count is the sum of the lengths of those runs.
        """
    ...
    def is_text_file(filepath: str) -> bool:
        """Heuristically determine whether ``filepath`` is a text file."""
        ...
        # No extension: sniff the first 1KB for null bytes (binary indicator).
        if not ext:
            try:
