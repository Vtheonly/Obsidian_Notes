...
This chapter is directly assessed by the quiz below. The quiz is the retrieval/practice layer for the concepts in this note: lexical grammar, tokenization, syntactic grammar, AST construction, Automatic Semicolon Insertion (ASI), restricted productions, LineTerminators, statement-continuation hazards, numeric-literal edge cases, and contextual interpretation of `/`.

**Quiz:** [Q1 — Syntax and Lexical Structure](../../Quiz/Q1.md)

The relationship is bidirectional: **chapter → quiz for practice** and **quiz → chapter for the source material**. The quiz declares this chapter in its `sources` metadata and includes a direct vault link back here.
