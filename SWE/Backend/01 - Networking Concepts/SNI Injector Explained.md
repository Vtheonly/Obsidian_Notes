...
- [[SSL and TLS]] - SNI injection works by manipulating the TLS handshake process, specifically the SNI extension in the ClientHello message. Understanding TLS is essential for understanding how and why SNI injection works.
- [[HTTP and HTTPS]] - The technique relies on the fact that HTTPS traffic (HTTP over TLS) is commonly allowed through firewalls on port 443, making it an ideal cover for tunneled traffic.
