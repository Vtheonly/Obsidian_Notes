# Advanced Layers and Image Efficiency
...
- **`python:3.12-alpine`:** Based on Alpine Linux. Tiny (~50MB).
  - *Warning:* Alpine uses a different C-library (`musl` instead of `glibc`). Some Python packages (like NumPy or Pandas) might fail to install or require complex compilation on Alpine.

---

Previous: [[4. Parent Images and Docker Hub]] | Next: [[5. The Dockerfile]]
