...
>> [!success]- Answer
>> False. Containers share the host machine's OS kernel. They do not boot their own kernel.
...
>> [!success]- Answer
>> True. Environment inconsistency is exactly the problem Docker containers solve.

> [!question] The only prerequisite needed on a computer to run a containerized application is the Docker platform itself.
>> [!success]- Answer
>> True. Docker is the sole requirement; the container brings its own runtime, dependencies, and code.

> [!question] Virtual Machines (VMs) are generally more lightweight and faster to start than containers.
>> [!success]- Answer
>> False. Containers are more lightweight and faster to start because they share the host OS kernel.

> [!question] Docker containers on a single host machine share the host's operating system kernel.
>> [!success]- Answer
>> True. This is the key architectural difference between containers and VMs.

> [!question] Using containers complicates the setup process for developers joining a new project.
>> [!success]- Answer
>> False. Containers simplify setup because developers only need Docker; all dependencies are packaged inside the container.

> [!question] A container packages an application's source code, but not its dependencies or runtime.
>> [!success]- Answer
>> False. A container bundles the source code, dependencies, runtime, and configuration together.
...
>> [!success]- Answer
>> False. Container consistency is equally valuable in production, ensuring the same environment tested in development is deployed.
...
>> [!success]- Answer
>> True. Isolation means each container has its own environment, preventing version conflicts.

> [!question] Containers consume significantly fewer resources than VMs, allowing more applications to run on a single host.
>> [!success]- Answer
>> True. Containers do not need a full guest OS, so they use far less CPU, memory, and disk.
