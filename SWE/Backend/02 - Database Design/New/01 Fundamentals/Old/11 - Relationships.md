...
Before defining relationships, it is necessary to identify the entities that will participate in them. An entity, as defined in [[8 - Database Terms]], is anything about which data is stored. In a typical database, there are multiple entity types, each represented by its own table. For example, a university database might include entities for students, professors, classes, and majors. Each entity has its own set of attributes that describe it.
...
The following sections explore each relationship type in detail, starting with one-to-one relationships. See [[12 - One-to-One Relationships]] for the first in-depth examination.
