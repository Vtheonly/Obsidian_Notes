...
**Important restriction:** You cannot use `SET NULL` on a foreign key column that has the `NOT NULL` constraint. If the column is declared `NOT NULL`, the database cannot set it to null, and the operation will fail with an error. See [[28 - NOT NULL Foreign Key]] for more on this interaction.
...
Cross-reference: [[27 - Foreign Key]], [[28 - NOT NULL Foreign Key]], [[7 - Data Integrity]]
