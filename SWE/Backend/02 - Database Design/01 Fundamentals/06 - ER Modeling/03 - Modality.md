...
Modality describes whether participation in a relationship is mandatory or optional. While cardinality specifies how many rows can be associated (one or many), modality specifies whether at least one row must be associated (zero or one as the minimum). Together, cardinality and modality fully define the constraints on a relationship between two tables in an ER diagram. See [[02 - Cardinality]] for the companion concept and [[01 - Introduction to ER Modeling]] for the broader context of ER modeling.
...
Cross-reference: [[01 - Introduction to ER Modeling]], [[02 - Cardinality]], [[07 - NOT NULL Foreign Key]]
