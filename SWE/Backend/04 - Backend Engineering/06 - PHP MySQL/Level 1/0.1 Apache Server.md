---
tags: [PHP, Apache, WebServer, LAMP, PHP-FPM, mod_php]
---

# Apache Server

Apache is not the PHP server. Apache is a *web server*. It needs PHP to handle `.php` files, but its main job is to handle web traffic, not execute PHP code.
...
See also: [[0. Execution Models of PHP]], [[0.2 PHP server]], [[0. Docker vs Apache]] (Production Mode Practices)
...
You need Apache (or another web server like Nginx) to be the public-facing, robust, secure front door for your website. It handles the chaos of the internet and only calls on the PHP "specialist" when a specific dynamic task is required.
