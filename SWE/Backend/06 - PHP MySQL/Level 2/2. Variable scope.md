...
See also: [[1. Functions]], [[4. define]] (Level 1)
...
## 5. The $GLOBALS Superglobal Array

PHP also provides a special array called `$GLOBALS`. This is an associative array that contains references to all variables available in the global scope. You can access and modify global variables using this array from anywhere, including inside functions.
...
?>
```
**Note:** While `$GLOBALS` can be used, the `global` keyword is generally preferred for clarity when you only need to access a few specific global variables.
...
-   **Use Constants for Global, Unchanging Values:** For truly global values that never change (like configuration settings), use PHP constants (`define()` or `const`) instead of global variables. Constants are globally accessible by default without needing `global`. See also: [[4. define]] (Level 1)
